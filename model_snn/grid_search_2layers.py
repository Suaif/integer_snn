...
def train_biograd_mnist(snn_param, hidden_dim_list, float_mode, feedback_weights, loss_mode, seed_list=0, 
    ...
    for seed in seed_list:
        ...
        session_name = f"grid_search_{'_'.join(exp_name.split('_')[:-1])}/mnist_biograd_"
        session_name += "float_" if float_mode else "int_"
        session_name += "aligned_" if feedback_weights else "transposed_"
        session_name += f"loss_{loss_mode}_{seed}_exp_{exp_name}"

        plot_title = f"{session_name}\n MNIST with BioGrad"

        online_snn = BioGradNetworkWithSleep(
            in_shape, out_dim, hidden_dim_list, snn_param, l2_loss, device,
            float_mode=float_mode, feedback_weights=feedback_weights, loss_mode=loss_mode, terminal_stats=False
        )
        ...
        train_acc, val_acc, test_acc, fb_angle, fb_ratio, best_stats = biograd_snn_training(
            ...
            test_batch_size=test_batch_size, epoch=epochs, save_epoch=save_epoch, lr=lr,
        ...
        save_path = f"./save_models/{session_name}/"
        os.makedirs(save_path, exist_ok=True)

        pickle.dump(train_acc, open(f"{save_path}train_accuracy_list.p", "wb+"))
        pickle.dump(val_acc, open(f"{save_path}val_accuracy_list.p", "wb+"))
        pickle.dump(test_acc, open(f"{save_path}test_accuracy_list.p", "wb+"))
        pickle.dump(fb_angle, open(f"{save_path}feedback_angle_list.p", "wb+"))
        pickle.dump(fb_ratio, open(f"{save_path}feedback_ratio_list.p", "wb+"))

        plot_training.plot_training(train_acc, val_acc, test_acc, f"{save_path}training_plot.png", title=plot_title)
        plot_training.plot_fb(fb_angle, fb_ratio, f"{save_path}feedback_plot.png", title=plot_title)
...
def train_and_evaluate(config):
    ...
    try:
        ...
        train_acc, val_acc, test_acc, _, _, best_stats, name = train_biograd_mnist(
            snn_params, params['hidden_dim_list'], params['float_mode'], params['feedback_weights'], params['loss_mode'], 
            seed_list=[params['seed']], num_bits=params['num_bits'], weight_init_bits=params['weight_init_bits'], 
            spike_stats=False, batch_stats=False, epochs=params['epochs'], exp_name=exp_name, data=params['data']
        )
...
def preload_data(batch_size, validation_size, test_batch_size, device="cuda"):
    """Preload MNIST data for training, validation, and testing."""
...
if __name__ == "__main__":
    ...
    print(f"Total combinations: {len(list(ParameterGrid(grid)))}")
