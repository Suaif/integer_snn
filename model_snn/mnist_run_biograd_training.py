...
if float_mode:
    ...
    out_layer_dict = {'Vdecay': 1., 'Vth': 0.3, 'Grad_win': 0.3, 'Grad_amp': 1., 'Fb_th': 1., 
                      'lr': 1e-3, 'Weight_decay': 0}
    hidden_layer_dict1 = {'Vdecay': 1., 'Vth': 0.3, 'Grad_win': 0.3, 'Grad_amp': 1.,
                          'lr': 1e-3, 'Weight_decay': 0}
    # hidden_layer_dict1 = {'Vdecay': 1., 'Vth': 0.3, 'Grad_win': 0.3, 'Grad_amp': 1.,
    #                       'lr': 1e-3, 'Weight_decay': 0}
    ...
    factor = 256.
    out_layer_dict = {'Vdecay': 0.5, 'Vth': factor*2000, 'Grad_win': factor*4000, 'Grad_amp': 1., 'Fb_th': 1., 
                      'lr': 2**-2, 'Weight_decay': 0.}
    hidden_layer_dict1 = {'Vdecay': 0.5, 'Vth': factor*500., 'Grad_win': factor*1000., 'Grad_amp': 1.,
                          'lr': 2**-18, 'Weight_decay': 0.}
...
# loss_fn, loss_parameters = 'positive', None
...
experiment_group = "mnist/final_results/"
...
with torch.no_grad():
    for seed in seed_list:
        ...
        np.random.seed(seed)
        session_name = f"mnist_biograd_quant_{str(len(snn_param['hidden_layer']))}_layers_"
