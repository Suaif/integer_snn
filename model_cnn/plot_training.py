...
def boxplot_weights(hidden_weights, output_weights, 
                    hidden_weights_quant, output_weights_quant,
                    deltas_list, deltas_list_abs,
                    title, save_path, best_epoch, plot=False):
    
    epochs = np.arange(len(output_weights))

    n_rows = len(hidden_weights) + 1
    ...
    # Hidden layers
    for i, (weights_quant, weights, deltas, deltas_abs) in enumerate(zip(hidden_weights_quant,
                                                                         hidden_weights, 
                                                                         deltas_list,
                                                                         deltas_list_abs)):
        
        plot_data = [weights_quant, weights, deltas, deltas_abs]
...
def plot_lr(lr_lists, title, save_path, best_epoch, plot=False):
    ...
    lr_names = ['Hidden Layer', 'Output Layer']
