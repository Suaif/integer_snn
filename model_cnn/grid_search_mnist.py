...
def train_biograd_shd(snn_param, float_mode, aligned_weights, loss_mode, seed_list=0, 
    ...
    for seed in seed_list:
        ...
        session_name = f"mnist_biograd_cnn_{str(len(snn_param['hidden_layer']))}_layers_"
...
def train_and_evaluate(config):
    ...
    try:
        ...
        hidden_layer_dict = {'Vdecay': params['vdecay'], 'Vth': f*params['hidden_param'][0], 'Grad_win': f*params['hidden_param'][1], 'Grad_amp': 1., 
                             'n_filters': params['n_filters'], 'kernel_size': params['kernel_size'], 'reduce_dim': params['reduce_dim'],
        ...
        train_acc, val_acc, test_acc, _, _, best_stats, name = train_biograd_shd(snn_params, params['float_mode'], 
...
def get_seeds_results(file_path):
    ...
    # Define hyperparameters to group by
    group_by_columns = [
    ...
    # Keep only columns with varying values
    # summary = summary.loc[:, (summary != summary.iloc[0]).any() | (summary.columns == 'id_count')]

    # Save the summary
    save_path = f"grid_results_seeds/mnist/"
...
if __name__ == "__main__":
    ...
    # Get summary results from seeds
