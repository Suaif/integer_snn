            ...
            if frames_number is not None:
                ...
                if os.path.exists(frames_np_root):
                    ...
                    with ThreadPoolExecutor(max_workers=configure.max_threads_number_for_datasets_preprocess) as tpe:
                        ...
                        print(f'Integrate train samples to frames and save to [{frames_np_train_root}].')
                        for i_train in range(h5_file['labels'].__len__()):
                            # print(f'Start to integrate [{i_train}]-th train sample to frames and save to [{frames_np_train_root}].')
                            integrate_events_file_to_frames_file_by_fixed_frames_number_shd(h5_file, i_train, frames_np_train_root, self.split_by, frames_number, self.get_W(), False)
                            #tpe.submit(integrate_events_file_to_frames_file_by_fixed_frames_number_shd, h5_file, i_train, frames_np_train_root, self.split_by, frames_number, self.get_W(), True)
                            # if i == 5:
                            #     break

                        h5_file = h5py.File(os.path.join(extract_root, 'shd_test.h5'))
                        print(f'Integrate test samples to frames and save to [{frames_np_test_root}].')
                        for i_test in range(h5_file['labels'].__len__()):
                            # print(f'Start to integrate [{i_test}]-th test sample to frames and save to [{frames_np_test_root}].')
                            tpe.submit(integrate_events_file_to_frames_file_by_fixed_frames_number_shd, h5_file, i_test,
                                       frames_np_test_root, self.split_by, frames_number, self.get_W(), False)
                            # if i == 5:
                            #     break
                    ...
                    print(f"{i_train+1} train samples and {i_test+1} test samples saved to [{frames_np_root}].")
                ...
                if os.path.exists(frames_np_root):
                    ...
                    with ThreadPoolExecutor(max_workers=configure.max_threads_number_for_datasets_preprocess) as tpe:
                        print(f'Start ThreadPoolExecutor with max workers = [{tpe._max_workers}].')
                        h5_file = h5py.File(os.path.join(extract_root, 'shd_train.h5'))
                        print(f'Integrate train samples to frames and save to [{frames_np_test_root}].')
                        for i_train in range(h5_file['labels'].__len__()):
                            # print(f'Start to integrate [{i_train}]-th train sample to frames and save to [{frames_np_train_root}].')
                            tpe.submit(integrate_events_file_to_frames_file_by_fixed_duration_shd, h5_file, i_train,
                                       frames_np_train_root, self.duration, self.get_W(), False)
                            # if i == 5:
                            #     break
                        ...
                        for i_test in range(h5_file['labels'].__len__()):
                            # print(f'Start to integrate [{i_test}]-th test sample to frames and save to [{frames_np_test_root}].')
                            tpe.submit(integrate_events_file_to_frames_file_by_fixed_duration_shd, h5_file, i_test,
                                       frames_np_test_root, self.duration, self.get_W(), False)
                            # if i == 5:
                            #     break
                ...
                if os.path.exists(frames_np_root):
                    ...
                    with ThreadPoolExecutor(max_workers=configure.max_threads_number_for_datasets_preprocess) as tpe:
                        ...
                        for i_train in range(h5_file['labels'].__len__()):
                            # print(f'Start to integrate [{i_train}]-th train sample to frames and save to [{frames_np_train_root}].')
                            tpe.submit(custom_integrate_function, h5_file, i_train,
                        ...
                        for i_test in range(h5_file['labels'].__len__()):
                            # print(f'Start to integrate [{i_test}]-th test sample to frames and save to [{frames_np_test_root}].')
                            tpe.submit(custom_integrate_function, h5_file, i_test,
