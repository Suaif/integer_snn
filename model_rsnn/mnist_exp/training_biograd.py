...
def biograd_snn_training(network, spike_ts, device, session_name,
    ...
    with torch.inference_mode():
        for ee in range(epoch):
            ...
            for n_layer, layer in enumerate(network.hidden_cells):
                ...
                train_stats_message += print_stats(layer.recurrent_func.weight.cpu().detach().numpy(),      f"Hidden {n_layer} Rec Q", False) + "\n"
            ...
            for data in val_dataloader:
                ...
                event_img = img_2_event_img(img, device, spike_ts)
                predict_label, act = network.test(event_img, spike_stats)
                val_activations += act
            ...
            time_message = "Training Time: %.1f Val Time %.1f Test Time %.1f" % (train_end - train_start, val_end - val_start, test_end - test_start)
