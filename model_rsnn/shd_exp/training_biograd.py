...
def biograd_snn_training(network, device, session_name,
                         ...
                         stats=False, float_mode=False, train_stats=True, data_params=0, lr_scheduler=0, 
    ...
    if data_params == 0: # Default data params
        data_type = 'frame'
        duration = None
        frames = 20
        split_by = 'time'
    ...
    if network.loss_mode != 'feedback':
        ...
        raise ValueError("Feedback loss mode: soft_error_step NEEDS TO BE UPDATED")
    ...
    hidden_weights_list, rec_weights_list, output_weights_list = [], [], []
    hidden_weights_quant_list, rec_weights_quant_list, output_weights_quant_list = [], [], []
    deltas_list, rec_deltas_list = [], []
    deltas_abs_list, deltas_abs_rec_list = [], []
    activation_list = [[[], []]]
    train_activation_fb_list = [[], []]
    max_volts_hist = torch.zeros(epoch, network.n_layers)
    lr_lists = []
    hidden_weight_activity, rec_weight_activity, out_weight_activity = [], [], []
    hidden_weight_quant_activity, rec_weight_quant_activity, out_weight_quant_activity = [], [], []
    volt_activity = [[],[]]
    ...
    # log_dir = os.path.join(tf_folder, "float_batch_2")
    ...
    hparams = {
        ...
        "hidden_rec_factor": float(network.hidden_cells[0].rec_factor),
        "hidden_lr": float(network.hidden_cells[0].lr),
        "hidden_lr_rec": float(network.hidden_cells[0].lr_rec),
    ...
    for n_layer, layer in enumerate(network.hidden_cells):
        summary_message += (f"Hidden Layer {n_layer}\n"
                        ...
                        f"Rec_factor {layer.rec_factor}, Vdecay_rec {layer.vdecay_rec}, Learning rate Rec {layer.lr_rec}\n")
        ...
        summary_message += print_stats(layer.recurrent_func.weight.cpu().detach().numpy(),      f"Rec  ", False) + '\n'
        summary_message += print_stats(layer.recurrent_func_quant.weight.cpu().detach().numpy(),f"Rec Q", False) + '\n'
        ...
        rec_weights_list.append([layer.recurrent_func.weight.data])
        hidden_weights_quant_list.append([layer.forward_func_quant.weight.data])
        rec_weights_quant_list.append([layer.recurrent_func_quant.weight.data])
        deltas_list.append([torch.zeros_like(layer.forward_func.weight)])
        rec_deltas_list.append([torch.zeros_like(layer.recurrent_func.weight)])
        deltas_abs_list.append([torch.zeros_like(layer.forward_func.weight)])
        deltas_abs_rec_list.append([torch.zeros_like(layer.recurrent_func.weight)])
        lr_lists.append([])
        lr_lists.append([])
    ...
    with torch.inference_mode():
        for ee in range(epoch):
            ...
            # if ee == 11:
            #     spike_stats, batch_stats = True, True
            # Epoch stats
            ...
            # Batch stats
            h_w_batch, rec_w_batch, output_w_batch = [], [], []
            h_w_q_batch, rec_w_q_batch, output_w_q_batch = [], [], []
            deltas_batch, deltas_rec_batch = [], []
            deltas_abs_batch, deltas_abs_rec_batch = [], []
            activation_batch = [[[]]]
            ...
            for layer in network.hidden_cells:
                ...
                rec_w_batch.append([copy.deepcopy(layer.recurrent_func.weight.data)])
                h_w_q_batch.append([copy.deepcopy(layer.forward_func_quant.weight.data)])
                rec_w_q_batch.append([copy.deepcopy(layer.recurrent_func_quant.weight.data)])
                ...
                deltas_rec_batch.append([torch.zeros_like(layer.recurrent_func.weight.data)])
                deltas_abs_rec_batch.append([torch.zeros_like(layer.recurrent_func.weight.data)])
            ...
            for n_batch, (x, labels, _) in enumerate(train_dataloader):
                ...
                predict_label, hid_fwd_states, hid_fb_states, out_fwb_state, out_fb_state, fb_step, max_volts, act, act_fb, volt_act = network.train_online(
                ...
                # Compute weight activity (non-zero weights)
                hidden_weight_activity.append([torch.sum(layer.forward_func.weight != 0).item() / layer.forward_func.weight.numel() for layer in network.hidden_cells])
                rec_weight_activity.append([torch.sum(layer.recurrent_func.weight != 0).item() / layer.recurrent_func.weight.numel() for layer in network.hidden_cells])
                out_weight_activity.append([torch.sum(network.output_cell.forward_func.weight != 0).item() / network.output_cell.forward_func.weight.numel()])

                hidden_weight_quant_activity.append([torch.sum(layer.forward_func_quant.weight != 0).item() / layer.forward_func.weight.numel() for layer in network.hidden_cells])
                rec_weight_quant_activity.append([torch.sum(layer.forward_func_quant.weight != 0).item() / layer.forward_func.weight.numel() for layer in network.hidden_cells])
                out_weight_quant_activity.append([torch.sum(network.output_cell.forward_func_quant.weight != 0).item() / network.output_cell.forward_func.weight.numel()])
                ...
                volt_activity[0].append(volt_act[0])
                volt_activity[1].append(volt_act[1])
                ...
                # show batch stats if recurrent weights contain nan values
                # if torch.isnan(network.hidden_cells[0].recurrent_func.weight).any():
                #     print("Recurrent weights have nan values")
                #     batch_stats = True
                ...
                if batch_stats:
                    ...
                    for n_layer, layer in enumerate(network.hidden_cells):
                        ...
                        print_stats(layer.recurrent_func.weight.cpu().detach().numpy(),              f"Hidden Layer {n_layer} Rec  ")
                        print_stats(layer.recurrent_func_quant.weight.cpu().detach().numpy(),        f"Hidden Layer {n_layer} Rec Q")
                ...
                if plot_batch:
                    for n_layer, layer in enumerate(network.hidden_cells):
                        ...
                        delta_abs_rec = layer.recurrent_func.weight.data - rec_w_batch[n_layer][-1]
                        deltas_abs_rec_batch[n_layer].append(delta_abs_rec)
                        delta = 100 * delta_abs / (h_w_batch[n_layer][-1])
                        delta = torch.nan_to_num(delta, nan=0.0, posinf=0.0, neginf=0.0)
                        delta_rec = 100 * delta_abs_rec / (rec_w_batch[n_layer][-1])
                        delta_rec = torch.nan_to_num(delta_rec, nan=0.0, posinf=0.0, neginf=0.0)

                        h_w_batch[n_layer].append(layer.forward_func.weight.data)
                        h_w_q_batch[n_layer].append(layer.forward_func_quant.weight.data)
                        rec_w_batch[n_layer].append(layer.recurrent_func.weight.data)
                        rec_w_q_batch[n_layer].append(layer.recurrent_func_quant.weight.data)
                        deltas_batch[n_layer].append(delta)
                        deltas_rec_batch[n_layer].append(delta_rec)
            ...
            for n_layer, layer in enumerate(network.hidden_cells):
                ...
                train_stats_message += print_stats(layer.recurrent_func.weight.cpu().detach().numpy(),      f"Hidden {n_layer} Rec  ", False) + "\n"
                train_stats_message += print_stats(layer.recurrent_func_quant.weight.cpu().detach().numpy(),f"Hidden {n_layer} Rec Q", False) + "\n"
                ...
                delta_abs_rec = layer.recurrent_func.weight.data - rec_w_batch[n_layer][0]
                deltas_abs_rec_list[n_layer].append(delta_abs_rec)
                delta = 100 * delta_abs / (h_w_batch[n_layer][0])
                delta = torch.nan_to_num(delta, nan=0.0, posinf=0.0, neginf=0.0)
                deltas_epoch[layer.name] = (delta, delta_abs, layer.lr)
                delta_rec = 100 * delta_abs_rec / (rec_w_batch[n_layer][0])
                delta_rec = torch.nan_to_num(delta_rec, nan=0.0, posinf=0.0, neginf=0.0)
                deltas_epoch[layer.name + " - Rec"] = (delta_rec, delta_abs_rec, layer.lr_rec)
                ...
                rec_weights_list[n_layer].append(layer.recurrent_func.weight.data)
                rec_weights_quant_list[n_layer].append(layer.recurrent_func_quant.weight.data)
                deltas_list[n_layer].append(delta)
                rec_deltas_list[n_layer].append(delta_rec)
                max_volts_hist[ee][n_layer] = max_volts_epoch[n_layer]
                lr_lists[n_layer*2].append(layer.lr)
                lr_lists[n_layer*2+1].append(layer.lr_rec)
            ...
            if writer_epoch:
                ...
                for n_layer, layer in enumerate(network.hidden_cells):
                    ...
                    tf_writer.add_scalar(f"Learning_rate/layer_{n_layer}_lr_rec", layer.lr_rec, ee)
                    ...
                    tf_writer.add_histogram(f"Weights_{n_layer}/recurrent_weights", layer.recurrent_func.weight, ee)
                    tf_writer.add_histogram(f"Weights_{n_layer}/recurrent_weights_quant", layer.recurrent_func_quant.weight, ee)
            ...
            plot_training.boxplot_weights(h_w_batch, rec_w_batch, output_w_batch,
                                            h_w_q_batch, rec_w_q_batch, output_w_q_batch,
                                            deltas_batch, deltas_rec_batch, deltas_abs_batch, deltas_abs_rec_batch,
    ...
    plot_training.boxplot_weights(hidden_weights_list, rec_weights_list, output_weights_list, 
                                  hidden_weights_quant_list, rec_weights_quant_list, output_weights_quant_list,
                                  deltas_list, rec_deltas_list, deltas_abs_list, deltas_abs_rec_list,
    ...
    # Merge weight activity lists to a big tensor and save it
    hidden_weight_activity = torch.tensor(hidden_weight_activity)
    rec_weight_activity = torch.tensor(rec_weight_activity)
    out_weight_activity = torch.tensor(out_weight_activity)
    hidden_weight_quant_activity = torch.tensor(hidden_weight_quant_activity)
    rec_weight_quant_activity = torch.tensor(rec_weight_quant_activity)
    out_weight_quant_activity = torch.tensor(out_weight_quant_activity)
    weight_activity = torch.cat((hidden_weight_activity, rec_weight_activity, out_weight_activity), dim=1)
    weight_quant_activity = torch.cat((hidden_weight_quant_activity, rec_weight_quant_activity, out_weight_quant_activity), dim=1)
    torch.save(weight_activity, os.path.join(session_file_dir, "weight_activity.pt"))
    torch.save(weight_quant_activity, os.path.join(session_file_dir, "weight_quant_activity.pt"))

    # same process with the activation lists
    activation_list = torch.tensor(activation_list)
    torch.save(activation_list, os.path.join(session_file_dir, "activation_list.pt"))

    volt_activity = torch.tensor(volt_activity)
    torch.save(volt_activity, os.path.join(session_file_dir, "volt_activity.pt"))
